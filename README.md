...
我的做法是增加了一些更细粒度的规则。当我们在 Codex、Claude Code 或终端软件内，需要输入 /usage 这类命令，或者输入一些 Skill 时，只要在开头输入"/"，就可以自动临时切换到英文模式。之后按回车、空格或 Tab，都能自动切换回中文。因为这些本来就是 AI Coding 场景里常见的调用结束符，所以不需要额外的认知和记忆负担，就能在“临时英文”和“常驻中文”之间无感切换。
